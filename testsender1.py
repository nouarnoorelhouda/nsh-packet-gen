...
def main():
    ...
    do = 'send'
    type = 'vxlan_gpe_nsh'
    outer_source_mac = 'fa:16:3e:75:0d:09'
    inner_source_mac = 'fa:16:3e:75:0d:09'
    outer_destination_mac = 'fa:16:3e:4f:47:eb'
    inner_destination_mac = 'fa:16:3e:4f:47:eb'
    outer_source_ip = '20.0.0.33'
    inner_source_ip = '20.0.0.33'
    outer_destination_ip = '20.0.0.179'
    inner_destination_ip = '20.0.0.179'
    outer_source_udp_port = 55651
    inner_source_udp_port = outer_source_udp_port
    inner_destination_udp_port = 25
    outer_destination_udp_port = inner_destination_udp_port
    number = 100
    swap_ip = True
    verbose = 'on'
    macaddr = None
    ...
    try:
        ...
        if ((do == "forward") or (do == "send")):
            ...
            send_s.bind((interface, 0))
        if interface is not None:
            macstring = getmac(interface)
            if (macstring is not None):
                macaddr = macstring.split(':')
        if (do == "send"):
            if (inner_source_mac is None):
                inner_source_mac = macstring
            if (inner_destination_mac is None):
                print("Error: you must specify inner destination MAC for packet send")
                sys.exit(-1)
            if (inner_source_ip is None) or (inner_destination_ip is None):
                print("Error: you must specify inner source IP and inner destination IP for packet send")
                sys.exit(-1)
            if (outer_source_mac is None):
                outer_source_mac = inner_source_mac
            if (outer_destination_mac is None):
                outer_destination_mac = inner_destination_mac
            if (outer_source_ip is None):
                outer_source_ip = inner_source_ip
            if (outer_destination_ip is None):
                outer_destination_ip = inner_destination_ip
            if (outer_source_udp_port is None):
                outer_source_udp_port = 55651
            if (inner_source_udp_port is None):
                inner_source_udp_port = outer_source_udp_port
            if (inner_destination_udp_port is None):
                inner_destination_udp_port = 25
            if (number is None):
                number = 10

    except OSError as e:
        print("{}".format(e) + " '%s'" % interface)
        sys.exit(-1)

    do_print = ((do != "forward") or (verbose == "on"))
    ...
    if (do == "send"):
        ...
        """ Set Ethernet header """
        dstmacaddr = outer_destination_mac.split(":")
        ...
        mynshcontextheader.network_platform = int_from_bytes(socket.inet_aton(outer_destination_ip))
        ...
        innerippack = build_udp_packet(inner_source_ip, inner_destination_ip, inner_source_udp_port, inner_destination_udp_port, "Hellow, World!!!".encode('utf-8'), False)
        if (type == "vxlan_gpe_nsh"):
            outerippack = build_udp_packet(outer_source_ip, outer_destination_ip, outer_source_udp_port, 4790, myvxlanheader.build() + mynshbaseheader.build() + mynshcontextheader.build() + myethheader.build() + innerippack, False)
        elif (type == "eth_nsh"):
    ...
    while True:
        ...
        if ((myethheader.ethertype0 == 0x89) or (myethheader.ethertype1 == 0x4f)):
            ...
            if ((do == "forward") and (interface is not None)):
        ...
        if (myudpheader.udp_dport in vxlan_gpe_udp_ports):
            ...
            if ((do == "forward") and (interface is not None) and (mynshbaseheader.service_index > 1)):
                """ Build IP packet"""
                if (myudpheader.udp_dport in vxlan_gpe_udp_ports):
                    """ nsi minus one """
                    mynshbaseheader.service_index = mynshbaseheader.service_index - 1
                    ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, myvxlanheader.build() + mynshbaseheader.build() + mynshcontextheader.build() + packet[eth_length+ip_length+udp_length+vxlan_length+nshbase_length+nshcontext_length:], swap_ip)
                else:
                    ippack = build_udp_packet(str(socket.inet_ntoa(pack('!I', myipheader.ip_saddr))), str(socket.inet_ntoa(pack('!I', myipheader.ip_daddr))), myudpheader.udp_sport, myudpheader.udp_dport, packet[eth_length+ip_length+udp_length:], swap_ip)
